...
const getElement = (selector) => {
  ...
  if (element){
    return element} // this is if element exist
  else {throw Error(
...
// API needed for index page to work
...
function addMeal (meal) {
  ...
  meal_card.innerHTML = `
  <figure class="meal-card-img-container">
    <img src="${meal.strMealThumb}" alt="${meal.strMeal}">
  </figure>
  <figcaption class="meal-name">
    <p>${meal.strMeal}</p>
...
function addMealToFav (meal) {
  ...
  // This gives the div element with the class of top
  // console.log(fav_meals.firstChild.nextSibling.firstChild.nextSibling)
...
function showMealPopup (meal) {
  ...
  const ingredients = [];
  // 20 is just the max size of ingredient in API
  for(let i = 1; i <= 20; i++) {
